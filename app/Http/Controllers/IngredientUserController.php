...
class IngredientUserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $user->allergy_info = $user->allergy_info . ($validated['allergy_info'] ? '; ' . $validated['allergy_info'] : '');
        $user->preference_info = $user->preference_info . ($validated['preference_info'] ? '; ' . $validated['preference_info'] : '');
        $user->save();

        // 保存完了後にリダイレクト
        return redirect()->route('ingredient_user.index')->with('success', 'アレルギーと好みの情報が新しく作成されました。');
    }
    ...
    public function update(Request $request, $userId)
    {
        ...
        $user->allergy_info = $request->input('allergy_info');
        $user->preference_info = $request->input('preference_info');
        $user->save();

        return redirect()->route('ingredient_user.edit', $user->id)
